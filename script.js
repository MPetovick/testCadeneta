...
function generatePDF() {
    ...
    try {
        ...
        if (appState.isTelegram) {
            // Telegram: generar PDF y abrir en el visor
            const pdfData = doc.output('blob');
            const file = new File([pdfData], `mnemoniqr-backup-${Date.now()}.pdf`, { type: 'application/pdf' });
            window.Telegram.WebApp.openDocument(file, 'application/pdf');
            showToast('PDF opened in Telegram viewer', 'success');
        } else {
...
function copyQRToClipboard() {
    ...
    if (appState.isTelegram) {
        // Telegram: enviar datos como imagen
        const qrDataUrl = dom.qrCanvas.toDataURL('image/png');
        window.Telegram.WebApp.sendData(qrDataUrl);
        showToast('QR data sent to Telegram', 'success');
    } else {
...
function downloadQRAsPNG() {
    ...
    try {
        if (appState.isTelegram) {
            // Telegram: descargar como archivo
            dom.qrCanvas.toBlob(blob => {
                const file = new File([blob], `mnemoniqr-${Date.now()}.png`, { type: 'image/png' });
                window.Telegram.WebApp.downloadFile(file, 'image/png');
                showToast('QR downloaded in Telegram', 'success');
            });
        } else {
...
function copySeedToClipboard() {
    const seedText = dom.decryptedSeed.value;
    if (appState.isTelegram) {
        window.Telegram.WebApp.sendData(seedText);
        showToast('Seed sent to Telegram', 'success');
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Solo mostrar modal de bienvenida fuera de Telegram
    if (!appState.isTelegram) {
